...
# Initialize OCR confidence display variables
ocr_confidence_display = ""  # Will hold the confidence string
...
while True:
    ...
    with torch.no_grad():
        prob_no_label = torch.sigmoid(model(img_t)).item()
        prob_label = 1 - prob_no_label
        pred = prob_label > 0.95  # or 0.5 if you prefer

    text_label = ""
    ocr_text = pytesseract.image_to_string(img)
    confidence, _ = calculate_ocr_confidence(ocr_text)
    ocr_confidence_display = f" | OCR Conf: {confidence:.0f}/100"

    if pred and confidence >= 65:
        print("Prescription detected!")
        print(f"THIS IS THE TEXT: {ocr_text}")
        print(f"THIS IS THE CONFIDENCE: {confidence}")
        text_label = f"Text: {ocr_text[:30]}..."  # Truncate long text
    elif pred:
        text_label = f"Text: {ocr_text[:30]}..."  # Still show text on camera

    # Show both probability and OCR confidence on the same line
    label = f"Prescription: {'YES' if pred else 'NO'} ({prob_label:.2f}){ocr_confidence_display}"
    color = (0, 255, 0) if pred else (0, 0, 255)
    cv2.putText(frame, label, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)

    # Show OCR text if available
    if text_label:
        cv2.putText(frame, text_label, (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
